...
interface IERC20 {
    ...
    event Burned(address indexed burner, address indexed burnTo, uint256 burnedAmountToken);
...
contract TokenName is Context, IERC20, Ownable, AntiWhale {
    ...
    uint256 private _totalburned =   20000 * (10**18);
    uint256 private _totaldev = 10000 * (10**18);
    uint256 private _totalmarketing = 10000 * (10**18);
    uint256 private _totalReflection = (MAX - (MAX % _totalSupply));
    uint256 private _totalFee;

    address payable public _marketing = payable(address(0xfD77A9e5dAF7F1F228A931652F25812e68f5273F));
    address payable public _dev = payable(address(0xfD77A9e5dAF7F1F228A931652F25812e68f5273F));
    address payable public _burn = payable(address(0x000000000000000000000000000000000000dEaD));

    bool public allocatedev = true;
    bool public allocatemarketing = true;
    bool public allocateburn = true;
    

    string private _name = "Merah Putih Token";
    ...
    constructor() {
        _rOwned[_msgSender()] = _totalReflection;
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02( 0x10ED43C718714eb63d5aA57B78B54704E256024E ); //Router Mainnet
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02( 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 ); //Router Testnet
        ...
        _allocateBurn(_msgSender());
        _allocateDev(_msgSender());
        _allocateMarketing(_msgSender());
    ...
    function _allocateDev(address sender) internal {
        if(allocatedev){
            _transfer(sender, _dev, _totaldev);
            allocatedev = false;
        }
    }

    function _allocateMarketing(address sender) internal {
        if(allocatemarketing){
            _transfer(sender, _marketing, _totalmarketing);
            allocatemarketing = false;
        }
    }

    function _allocateBurn(address sender) internal {
        if(allocateburn){
            _transfer(sender, _burn, _totalburned);
            emit Burned(sender, _burn, _totalburned);
            allocateburn = false;
        }
    }
